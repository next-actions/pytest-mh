...
* **Does your program affect the host in any way?** If yes, it is safer to run it in
  virtual machine or in container to avoid affecting your local host.
...
* **Does your program communicate with multiple backends?** If yes, you need to
  be able to assign each test to specific backend and also be able to reuse
...
* **Do you need complex tests that changes state of the system, file system or
  ...
  are reverted when a test is done so they do not affect other tests.
  ``pytest-mh`` takes care of that.
* Does your program **talk to LDAP/IPA/AD/Samba/Keberos**? If yes, ``pytest-mh``
...
* **Do you use** `pytest-multihost
  ...
  ``pytest-multihost`` plugin. It builds up on all its features and takes it to
  a whole new level. Therefore you definitely want to switch to ``pytest-mh``,
  however it is not backwards compatible.

When I don't want to use it?
****************************

* Do you want to using test your Python code? Then this plugin will not help
...
* Allows you to **define your own roles a provide fully typed API** to your
  tests that fulfils all your needs.
...
* Defines an available **multihost topology** - what roles are available in your
  current setup.
* **Associates each test with certain topology** - that is what roles are
  required to run the test.
* Supports **topology parametrization** - single test can run on multiple
  topologies.
